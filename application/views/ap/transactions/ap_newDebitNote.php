   ...
   ///});
				...
				function getItemCode(me){
					$.ajax({
						...
						url: "<?php echo base_url() ?>item/getItemCodes",
						success: function(data) {
				...
				function openModalAddSupplier(){
					$.ajax({
						type: "POST",
						url: "<?php echo base_url() ?>aptransaction/getSupplierNewForm",
				 ...
				 if( $("#parchaseDetailForm").validationEngine('validate')){
					if ( $("#purchaseInvoiceId").val()=="null"){
						...
						data: {
						 ...
						 url: "<?php echo base_url() ?>aptransaction/saveNewParchaseDebit",
						 success: function(data) {
					 	 var resul = eval ("(" + data + ")"); 
						  ...
						  $(''+
						   '<tr>' +
							  ...
							  '<td>'+
							  '<button onclick="deleteCompanyDetail(this)" data-detailid='+ resul.detailID +' class="btn btn-small btn-danger" type="button">Delete</button>'+
							  '</td>'+
						      '</tr>'
						...
						url: "<?php echo base_url() ?>aptransaction/saveNewDebitDetail",
						success: function(data) {
						 msg (1,"alert alert-success","Details successfully saved");
					 	  var resul = eval ("(" + data + ")"); 
					   $(''+
...
'<td class="span1"><input type="text" id="itemcode" name="itemcode" onkeyup="getItemDetails(this);" onchange="getItemDetails(this);" data-source="<?php echo $itemCodes; ?>" data-items="4" data-provide="typeahead" class="span12"></td>'+
                        ...
                        '<td class="span1"><input id="itemAmountET" disabled name="itemAmountET" onkeydown="return false;" class="span12 validate[required]" type="text" placeholder="Amount (Exclude Tax)"></td>'+
                        '<td class="span1"><div  class=" span12 input-prepend"><input data-taxid="" id="itemTax" onkeyup="calculamountET()" name="itemTax" class="input-mini validate[required, custom[number]]" type="text" placeholder="Tax(%)"><span id="displayTaxCode" onclick="taxPopup ()" style="margin-top:0;" class="btn add-on">....</span></div></td>'+
                        '<td class="span1"><input id="itemTaxAmount" disabled name="itemTaxAmount" onkeydown="return false;" class="span12" type="text" placeholder="Tax Amount"></td>'+
                        '<td class="span1"><input id="itemAmountIT" disabled name="itemAmountIT" onkeydown="return false;" class="span12" type="text" placeholder="Amount (Include Tax)"></td>'+
                        '<td class="span1">'+
						 	'<button class="btn btn-small btn-primary" onclick="saveParchaseRow()" type="button">Save</button>'+
						 	'<button id="btnCancel" class="btn btn-small" onclick="delRowInvoice(this)" type="button">Cancel</button>'+
						'</td>'+
                    '</tr>'
					  ).insertBefore( "#trInfos" );
...
function checkformNumber(me){
	...
	$.ajax({
	  type: "POST",
	  data: "code="+ me.value,
	  url: "<?php echo base_url() ?>aptransaction/checkformNumber",
	  success: function(data) {
		  if(data=="yes"){
...
<form id="parchaseDetailForm" action="#">    
	<div class="row-fluid">
		<div class="alert alert-info">
			<strong><?php echo $this->lang->line('titleDebitNote') ?></strong>
		</div>
	</div>
	  ...
	  <div class="span6 scCol">
		<div class="block" id="grid_block_1">
				...
				<ul class="dropdown-menu">
      			<!-- dropdown menu links -->
      				<li><a target="_blank" href="<?php echo base_url() ?>supplier/SupplierNew"  id="btnNewSupplier">
                    	<i class="icon-plus-sign"></i> <?php echo $this->lang->line('addNewSupplier')?></li>
          ...
          <div class="controls-row">
         	...
         	<div class="span4">
                  <input class="input-small validate[required]" id="formNo" name="formNo" value='<?php echo "$formNo"; ?>' style="color:red;"   type="text" placeholder="Form number ...">
            </div>
         	<div class="span4">
