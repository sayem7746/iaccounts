...
$(document).ready(function() {
   ...
   $("#test").dataTable( {
    ...
    $("#row_edit").dialog({
        ...
        buttons: {
            "Continue...": function() {
				window.location.href = "<?php echo base_url(); ?>supplier/shiptoNew/" + companyID + "/" + ID;		
                $(this).dialog("close");
            },
            Cancel: function() {
                $(this).dialog("close");
    ...
    function remove_row(row){
 		$.ajax({
  			dataType: "json",
  			type: "POST",
  			url: "<?php echo base_url(); ?>supplier/address_delete",
			data:"ID="+ID,
				...
				<div class="content np"> <!-- Content 1 -->   
						<?php
							$data = array(
									...
									'type'=>'hidden'); 
							if($suppliermaster){		
								echo form_input($data, $suppliermaster->ID); 
							}else{
        	...
        	<div class="block">
            	<div class="head">
                	<h2><?php echo $this->lang->line('titlelist') ?></h2>
                    	<ul class="buttons">
                        <li><a href="<?php echo base_url(); ?>supplier/shiptoNew/<?php if($suppliermaster) echo $suppliermaster->ID?>" class="" title="<?php echo $this->lang->line('addnew') ?>"><span class="i-plus-2"></span></a></li>
                        </ul>                                        
                </div>
