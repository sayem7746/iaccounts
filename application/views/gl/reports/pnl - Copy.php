	...
	<div class="wrap">    
		<div class="row-fluid">
			...
			<?php 
                 	...
                 	<?php }
					$total_credit = $total_credit + $row->currentyearcredit - $row->currentyeardebit;
					$total = $total + $row->currentyearcredit - $row->currentyeardebit;
					?>
			   	<?php 	}
				endforeach;?>
          <?php if($groupdetails){ 
						...
						foreach($groupdetails as $rowdet1):
                			...
                			<?php } 
								foreach($groupdetails as $rowdet2):
                                    ...
                                    <?php } ?>
                				<?php foreach($accountdetails as $row):
										if($row->acctGroupID == $rowdet2->ID){?>
                                <?php 
											if($row->currentyeardebit != '' && $row->currentyearcredit !=''){ ?>
                                        	<?php $totalamount = $row->currentyearcredit - $row->currentyeardebit ?>
            									<tr>
                									<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
													<?php echo substr($row->acctCode,0, 4).'-'.substr($row->acctCode,4, 3).'-'.substr($row->acctCode,7, 2) .' [ '.$row->acctName.' ]'?></td>
                									<td class="tar"><?php echo ($totalamount < 0 )?'('.number_format($totalamount * -1,2).')':number_format($totalamount,2) ?>&nbsp;&nbsp;</td>
                								</tr>
                                            <?php
												$subtotal_debit = $subtotal_debit + $row->currentyeardebit;
												$subtotal_credit = $subtotal_credit + $row->currentyearcredit - $row->currentyeardebit;
												$total_credit = $total_credit + $row->currentyearcredit - $row->currentyeardebit;
												$total = $total + $row->currentyearcredit - $row->currentyeardebit;
												?>
			            			<?php 	} 
							...
							?>
			            		<?php 	} 
									...
									endforeach;?>
            						<tr>
                						<td><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $this->lang->line($rowdet1->acctGroupName)?> <?php echo $this->lang->line('subtotal') ?></strong></td>
                						<td class="tar"><?php echo ($subtotal_credit < 0 )?'('.number_format($subtotal_credit * -1,2).')':number_format($subtotal_credit,2) ?>&nbsp;&nbsp;</td>
                					</tr>
			            <?php 	}
						endforeach;?>
                				<?php foreach($accountdetails as $row):
							...
							?>
								...
								endforeach;?>
            					<tr>
                					<td><strong>&nbsp;&nbsp;&nbsp;<?php echo $this->lang->line($rowdet->acctGroupName)?> <?php echo $this->lang->line('subtotal') ?></strong></td>
                					<td class="tar"><?php echo ($total < 0 )?'('.number_format($total * -1,2).')':number_format($total,2) ?>&nbsp;&nbsp;</td>
                				</tr>
            		<?php 	}
							endforeach; }?>
            					<tr>
                					<td><h5>&nbsp;<?php echo $this->lang->line($datatbl->acctGroupName)?> <?php echo $this->lang->line('total') ?></h5></td>
                					<td class="tar"><?php echo ($total_credit < 0 )?'('.number_format($total_credit * -1,2).')':number_format($total_credit,2) ?>&nbsp;&nbsp;</td>
                				</tr>
            <?php endforeach; ?>
            					<tr>
