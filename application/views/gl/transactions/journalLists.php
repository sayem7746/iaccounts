   ...
   var oTable = $("#test").dataTable( {
        "iDisplayLength": 10,
		 ...
		 "sPaginationType": "full_numbers", "aoColumns": [ { 
			...
			"bSortable": false } ]});
     oTable.fnSort( [ [1,'asc'] ] );
    $("table.editable").on("click",".reverse",function(){
    ...
    $("#row_edit").dialog({
        ...
        buttons: {
            "Continue...": function() {
				window.location.href = "<?php echo base_url();?>GLtransaction/jlReverse/" + fldid;		
                $(this).dialog("close");
            },
            Cancel: function() {
                $(this).dialog("close");
...
function changeList(){
	...
	var urls = "<?php echo base_url();?>GLtransaction/reverse/"+addyear+"/"+addper;
		...
		<div class="row-fluid">
			...
			<?php $bil = 0; 
            	...
            	<tr>
                    ...
                    <td class="tac"><?php 
							$js = 'class="btn btn-mini btn-info reverse" title="'.$this->lang->line("reverse").'"';
							echo form_button('reverse','<span class="i-previous">',$js); ?>
                    </td>
                </tr>
            <?php endforeach; }?>
                     </tbody>
