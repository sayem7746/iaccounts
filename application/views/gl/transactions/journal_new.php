...
//						if(i == 0)
					...
					$('#journal_number').removeAttr('disabled');
...
else if(toBeAdded>100)
{
  ...
  for (var i = 1; i <= toBeAdded; i++) {
    ...
    $("#oTable tbody").append(
        ...
        "<td class='span4 tar'><input type='text' name='amountdr["+rowArrayId+"]' class='span12 validate[custom[integer]] tar' onBlur='calculate_amount()'/></td>"+
        "<td class='span4 tar'><input type='text' name='amountcr["+rowArrayId+"]' class='span12 validate[custom[integer]] tar' onBlur='calculate_amount()'/></td>"+
            	...
            	<h2><?php echo $this->lang->line('details') ?></h2>
                	<ul class="buttons" id="btnAddnew">
                 		<li><button id="addnew" onclick="addRow()" class="btn btn-small btn-primary" type="button" title="<?php echo $this->lang->line('addnew') ?>"><i class="icon-plus-sign"></i></button>
                            <input id="numberofrows" type="text" class="input-mini tar" value="4"> <?php echo $this->lang->line('row') ?></li>
                	</ul>                                        
        	</div>
                            	<?php 								
								$data = array(
