...
$(document).ready(function() {
    ...
    $("table.editable td").dblclick(function () {        
        ...
        if(fieldname == 'routing_approver'){
			/*	$.ajax({
                type: "POST",
                url: "../admin/usermaster_allopt",
                success: function(data)
                {
					var opt = '<option>-- Please Select --</option>';
                    $.each(data,function(id,row)
                    {
						if(row.userid == eContent){
							opt += "<option value='" + row.userid + "' selected>" + row.username + "</option>";
						}else{
							opt += "<option value='" + row.userid + "'>" + row.username + "</option>";
						}
                    });
				eCell.addClass("editing");        
        		eCell.html('<select name=routing_approver>' + opt + '</select>');
                }        
            });	*/	}else{
			eCell.addClass("editing");        
		...
		<div class="span12">
			...
			<?php $bil = 0; 
            	...
            	<tr>
                	...
                	<td id="routing_code"><?php echo $datatbl->routing_code; ?></td>
                	<td id="routing_desc"><?php echo $datatbl->routing_desc; ?></td>
                	<td id="routing_approver"><?php echo $datatbl->routing_approver; ?></td>
                	<td id="routing_seq"><?php echo $datatbl->routing_seq; ?></td>
                    <td class="tac"><?php
