...
$(document).ready(function() {
    ...
    $("#row_edit").dialog({
        ...
        buttons: {
            "Continue...": function() {
				window.location.href = "<?php echo base_url() ?>PO_Transaction/edit/" + ID;		
                $(this).dialog("close");
            },
            Cancel: function() {
                $(this).dialog("close");
...
function changeList(){
	...
	var urls = "<?php echo base_url();?>PO_Transaction/poList/<?php echo $mode;?>/"+selDateFrom+"/"+selDateTo;
		window.location = urls;
}

function printPurchaseOrderList(){
	var selDateFrom = $('#selDateFrom').val();
	var selDateTo = $('#selDateTo').val();
	window.open("<?php echo base_url();?>PO_Transaction/poList_print/<?php echo $mode;?>/"+selDateFrom+"/"+selDateTo);
}
        ...
        if($(this).is(':checked')){
			...
			var postData = {
 			...
 			$.ajax({
  				type: "POST",
	  			dataType: "json",
				url: "<?php echo base_url()?>PO_Transaction/poList/<?php echo $mode;?>/",
				data: postData ,
	  			async: false,
        	...
        	<div class="block">
            	...
            	<div class="head">
                	<h2><?php echo $this->lang->line('titlelist');?></h2>
                    	<ul class="buttons">
                        	<li><a href="" name="updatePosting" onclick="printPurchaseOrderList()">Print</a></li>
            	...
            	<table cellpadding="0" cellspacing="0" width="100%" id="test" class="editable">
                    ...
                    <tr>
            			...
            			<th><?php echo $this->lang->line('salesAgentName');?></th>
                        <th class="tac"><?php echo $this->lang->line('poDate');?></th>
                		<?php
							if ($mode == "edit"){ 
